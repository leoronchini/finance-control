...
export default function SummaryCards({ summary }) {
  if (!summary) return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
      {[1,2,3,4].map(i => <div key={i} style={cardStyle}><div style={{ height: 60, background: 'var(--bg3)', borderRadius: 8 }} /></div>)}
    </div>
  )

  const reembolsos = summary.total_reembolsos ?? 0
  const saldo = summary.saldo ?? ((summary.total_entradas ?? 0) + reembolsos - (summary.total_saidas ?? 0))
